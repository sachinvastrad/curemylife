...
( cd backend
  ...
  # The Service Catalog (v1.1) adds: services, service_specialities, service_requests
  # and makes appointments.caseId nullable + adds appointments.serviceRequestId.
  npx prisma db push
...
# has finished bootstrapping). Probe the new Service Catalog endpoint —
...
if [ -z "$HEALTH_OK" ]; then
  echo -e "\033[1;31mWARN: API did not respond to /api/services within 15s — check 'pm2 logs curemylife-api'.\033[0m" >&2
fi
